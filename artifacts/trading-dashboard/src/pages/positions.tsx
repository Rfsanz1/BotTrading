...
export default function Positions() {
  ...
  const { data: posHttp, loading, refetch } = useApiData<Position[]>('/api/trading/positions/open', 30_000);
  ...
  const closePosition = async (position: Position) => {
    if (!position.id) {
      toast.error('This position has no canonical backend ID; close is unavailable.');
      return;
    }
    if (!confirm(`Close position for ${position.symbol}?`)) return;
    try {
      await apiFetch(`/api/trading/positions/${position.id}/close`, {
        method: 'POST',
        body: JSON.stringify({ reason: 'DASHBOARD_MANUAL_CLOSE' }),
      });
