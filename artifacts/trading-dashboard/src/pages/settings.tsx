...
export default function Settings() {
  ...
  const configData = unavailable<ConfigData>();
  const backups: any[] = [];
  const auditLogs: any[] = [];
  ...
  const saveConfig = async () => {
    toast.error('Runtime configuration is not exposed by the current backend; no request was sent.');
  };

  const createBackup = async () => {
    toast.error('Backups are not exposed by the current backend; no request was sent.');
  };
