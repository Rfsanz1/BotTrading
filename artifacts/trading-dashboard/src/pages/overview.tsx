...
export default function Overview() {
  ...
  const { data: statusHttp, refetch: refetchStatus } = useApiData<any>('/health/readiness', 30_000);
  ...
  const togglePause = async () => {
    toast.error('Pause/resume is not exposed by the current backend; no action was sent.');
  };

  const closeAll = async () => {
    toast.error('Close All is not exposed by the current backend; no action was sent.');
  };
