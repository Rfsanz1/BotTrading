...
async function bootstrap() {
  ...
  app.getHttpAdapter().get('/metrics', async (req: any, res: any) => {
    try {
      ...
      if (!secret || !header || !/^Bearer\s+\S+$/.test(header)) {
        return res.status(401).send({ ok: false, error: 'Authentication is required' });
      }
      verifyAccessToken(header.slice(7), secret);
      res.set('Content-Type', promClient.register.contentType);
      res.send(await promClient.register.metrics());
    } catch {
      res.status(401).send({ ok: false, error: 'Invalid or expired access token' });
    }
  });
