...
export class TradingController {
  constructor(
    ...
    private readonly paperSmokeService: PaperSmokeService,
    private readonly tradingLifecycleService: TradingLifecycleService,
    private readonly paperFailureService: PaperFailureService,
  ) {}

  @Post('paper-smoke')
  @HttpCode(200)
  async paperSmoke(@Request() req: any): Promise<SuccessResponseDTO<Record<string, unknown>>> {
    if (process.env.TRADING_MODE !== 'PAPER') {
      throw new BadRequestException('PAPER smoke fixture is only available in PAPER mode');
    }
    if (req.headers['x-paper-test-control'] !== 'smoke') {
      throw new BadRequestException('Explicit PAPER smoke test authorization is required');
    }
    await this.tradingLifecycleService.clearPaperStateForTest();
    return {
      success: true,
      data: await this.paperSmokeService.run(),
      message: 'PAPER smoke fixture completed',
    };
  }

  @Post('paper-restart')
  @HttpCode(200)
  async paperRestart(@Request() req: any): Promise<SuccessResponseDTO<Record<string, unknown>>> {
    if (process.env.TRADING_MODE !== 'PAPER') {
      throw new BadRequestException('PAPER restart test is only available in PAPER mode');
    }
    if (req.headers['x-paper-test-control'] !== 'restart') {
      throw new BadRequestException('Explicit PAPER restart test authorization is required');
    }
    await this.tradingLifecycleService.clearPaperStateForTest();
    return {
      success: true,
      data: await this.paperSmokeService.runRestartRecovery(() => this.tradingLifecycleService.recover()),
      message: 'PAPER in-process lifecycle recovery completed',
    };
  }

  @Post('paper-reconnect')
  @HttpCode(200)
  async paperReconnect(@Request() req: any): Promise<SuccessResponseDTO<Record<string, unknown>>> {
    if (process.env.TRADING_MODE !== 'PAPER') {
      throw new BadRequestException('PAPER reconnect test is only available in PAPER mode');
    }
    if (req.headers['x-paper-test-control'] !== 'reconnect') {
      throw new BadRequestException('Explicit PAPER reconnect test authorization is required');
    }
    await this.tradingLifecycleService.clearPaperStateForTest();
    return {
      success: true,
      data: await this.paperSmokeService.runReconnectRecovery(
        () => this.tradingLifecycleService.paperDisconnectForTest(),
        () => this.tradingLifecycleService.paperReconnectForTest(),
      ),
      message: 'PAPER reconnect recovery completed',
    };
  }

  @Post('paper-failure')
  @HttpCode(200)
  async paperFailure(@Request() req: any): Promise<SuccessResponseDTO<Record<string, unknown>>> {
    if (process.env.TRADING_MODE !== 'PAPER') throw new BadRequestException('PAPER failure test is only available in PAPER mode');
    if (req.headers['x-paper-test-control'] !== 'failure') {
      throw new BadRequestException('Explicit PAPER failure test authorization is required');
    }
    await this.tradingLifecycleService.clearPaperStateForTest();
    return { success: true, data: await this.paperFailureService.run(), message: 'PAPER failure test completed' };
  }
  ...
  async createOrder(
    ...
    try {
      ...
      const orderId = await this.tradingService.createOrder({ ...(body as any), userId });
      return {
        success: true,
        data: { id: orderId },
  ...
  async submitToExchange(@Param('orderId') orderId: string, @Request() req: any): Promise<SuccessResponseDTO<any>> {
    try {
  ...
  @ApiResponse({ status: 404, description: 'Order not found', type: ErrorResponseDTO })
  async cancelOrder(@Param('orderId') orderId: string, @Request() req: any): Promise<SuccessResponseDTO<any>> {
    try {
      await this.tradingService.assertOrderOwner(orderId, this.getUserId(req));
  ...
  @ApiResponse({ status: 404, description: 'Order not found', type: ErrorResponseDTO })
  async getOrder(@Param('orderId') orderId: string, @Request() req: any): Promise<SuccessResponseDTO<any>> {
    try {
      await this.tradingService.assertOrderOwner(orderId, this.getUserId(req));
  ...
  async recordTrade(@Body() body: RecordTradeDTO, @Request() req: any): Promise<SuccessResponseDTO<{ id: string }>> {
    try {
      await this.tradingService.assertOrderOwner(body.orderId, this.getUserId(req));
      const tradeId = await this.tradingService.recordTrade({
        orderId: body.orderId,
        executionConfirmation: await this.tradingService.verifyExecution(body.orderId),
      });
      return {
        success: true,
        data: { id: tradeId },
  ...
  async getPositionMetrics(
    ...
    try {
      await this.tradingService.assertPositionOwner(positionId, this.getUserId(req));
  ...
  async closePosition(
    ...
    try {
      await this.tradingService.assertPositionOwner(positionId, this.getUserId(req));
      const result = await this.tradingService.requestPositionClose(positionId, body.reason);
      return {
        success: true,
        data: { positionId, ...result },
        message: 'Exit order submitted; position closes only after an authoritative exchange fill',
      };
    } catch (error) {
      throw new BadRequestException(
