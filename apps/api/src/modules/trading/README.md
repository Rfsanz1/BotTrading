...
BINANCE_TESTNET_API_KEY=<your-testnet-key>  # TESTNET-only external secret
BINANCE_TESTNET_API_SECRET=<your-testnet-secret>  # TESTNET-only external secret
BINANCE_LIVE_API_KEY=<your-live-key>  # Separate LIVE secret; never reuse TESTNET
BINANCE_LIVE_API_SECRET=<your-live-secret>
EXCHANGE_CREDENTIAL_ENCRYPTION_KEY=<32-byte external master key>

Database `ApiKey.secretEncrypted` must contain the versioned AES-256-GCM
ciphertext produced by `CredentialCryptoService`; plaintext secrets are rejected.

Exchange-native SL/TP is intentionally fail-closed while the active adapter
does not support protection-order create/amend/cancel. Requests are rejected,
no local protection is persisted, and no active exchange protection is claimed.
