import { Controller, Post, Get, Param, Body, Req, HttpCode, Query, UnauthorizedException, ConflictException } from '@nestjs/common';
...
import { requireUserScope } from '../../common/user-scope';
...
export class AlertController {
  ...
  async webhook(@Body() payload: Record<string, any>, @Req() req: any): Promise<{ id: string; ok: boolean }> {
    ...
    if (!webhookId || !verifyWebhookSignature(
      rawBody,
    ...
    if (!userId) throw new UnauthorizedException('Webhook user mapping is not configured');

    try {
      const alertId = await this.alertService.handleWebhook(userId, payload, webhookSource, webhookId);
      return { id: alertId, ok: true };
    } catch (error) {
      if (error instanceof Error && error.message.includes('webhookId')) {
  ...
  async getUserAlerts(
    ...
    const alerts = await this.alertService.getUserAlerts(await requireUserScope(req, userId), limit, offset);
