...
import { ConsensusAnalysis, IConsensusBuilder } from '../../domain/interfaces';
...
export class ConsensusService implements IConsensusBuilder {
   ...
   */
  private calculateVotes(analyses: ConsensusAnalysis[]): Record<string, string> {
   ...
   */
  private calculateConfidenceScore(analyses: ConsensusAnalysis[]): number {
    if (analyses.length === 0) return 0;

    // Average confidence from all providers
    const avgConfidence = analyses.reduce((sum, analysis) => sum + Number(analysis.confidence), 0) / analyses.length;
   ...
   */
  private calculateRiskScore(analyses: ConsensusAnalysis[]): number {
    const riskMap = { HIGH: 0.8, MEDIUM: 0.5, LOW: 0.2 };
    
    const avgRisk = analyses.reduce((sum, analysis) => {
      const riskValue = riskMap[analysis.riskLevel as keyof typeof riskMap] || 0.5;
   ...
   */
  private generateBulletPoints(analyses: ConsensusAnalysis[]): string[] {
    const points: string[] = [];
   ...
   */
  private generateAnalysisSummary(analyses: ConsensusAnalysis[], recommendation: string): string {
