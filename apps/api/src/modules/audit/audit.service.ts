...
import { AIProvider, RecommendationType } from '@prisma/client';
...
export class AuditService implements IAuditLogger {
   ...
   * Log action
   */
  async log(params: { userId: string; action: string; resource: string; changes?: Record<string, any>; ipAddress?: string }, actorUserId?: string): Promise<void> {
    try {
      const meta = { ...(params.changes || {}), ...(actorUserId ? { actorUserId } : {}) };
  ...
  async logSystem(params: { userId: string; action: string; resource: string; changes?: Record<string, any>; ipAddress?: string }): Promise<void> {
   ...
   * Log error
   */
  async logError(params: { userId: string; action: string; error: string; ipAddress?: string }, actorUserId?: string): Promise<void> {
    try {
      await prisma.auditLog.create({
        data: {
          ...
          action: params.action,
          resource: 'ERROR',
          meta: { error: params.error, ...(actorUserId ? { actorUserId } : {}) },
          ip: params.ipAddress,
        },
      });
