...
LEGACY / NOT AUTHORIZED FOR ACTIVE DEPLOYMENT:
This Python runtime has direct Binance/OCO submission paths and is not part of
the NestJS canonical execution boundary. Do not deploy it as an active trading
runtime until its order flow is migrated through the canonical gate.
"""
...
@dataclass
class TradeDecision:
    """Canonical trade decision contract shared across market analysis layers.
    A snapshot of the decision must be persisted whenever a trade is opened or
    monitored so downstream execution, risk, and learning systems all consume the
    same contract.
    """
    id: str
    timestamp: str
    ...
    calibratedProbability: float = 0.0
    expectedValue: float = 0.0
    confidence: float = 0.0
    calibratedConfidence: float = 0.0
    riskLevel: str = "medium"
    positionSize: float = 0.0
    riskAmount: float = 0.0
    portfolioHeat: float = 0.0
    slippageEstimate: float = 0.0
    feeEstimate: float = 0.0
    reasoning: str = ""
    ...
    historicalStats: dict[str, Any] = field(default_factory=dict)
    dataQuality: float = 1.0
    decisionVersion: str = "1.0"
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "calibratedProbability": self.calibratedProbability,
            "expectedValue": self.expectedValue,
            "confidence": self.confidence,
            "calibratedConfidence": self.calibratedConfidence,
            "riskLevel": self.riskLevel,
            "positionSize": self.positionSize,
            "riskAmount": self.riskAmount,
            "portfolioHeat": self.portfolioHeat,
            "slippageEstimate": self.slippageEstimate,
            "feeEstimate": self.feeEstimate,
            "reasoning": self.reasoning,
            "keyFactors": self.keyFactors,
            "invalidationConditions": self.invalidationConditions,
            "supportingModels": self.supportingModels,
            "opposingModels": self.opposingModels,
...
LEGACY_EXECUTION_DISABLED: bool = True

def _reject_legacy_execution(operation: str) -> None:
    raise RuntimeError(
        f"Legacy Python execution is disabled: {operation}; "
        "use the NestJS canonical execution boundary"
    )
...
def execute_dca_buy(symbol: str, amount_usdt: float) -> dict:
    ...
    _reject_legacy_execution("DCA buy")
    try:
        base = ("https://testnet.binance.vision" if BINANCE_TESTNET
                else "https://api.binance.com")
...
def api_tradingview_webhook():
    ...
    def _run_tv_signal():
        try:
            ...
            trade_decision = ask_ai_orchestrated(symbol, df_1m)
            process_signal(symbol, trade_decision, price, atr, df_1m)
        except Exception as e:
...
def _canonical_regime_name(regime_value: Optional[object]) -> str:
    """Normalisasi nama regime ke bentuk canonical yang konsisten.
    Mendukung berbagai naming lama / baru agar kompatibel dengan kode lama dan
    requirement baru yang memakai TREND_UP / RANGE / BREAKOUT_*.
    """
    if regime_value is None:
        return "RANGE"
    if isinstance(regime_value, dict):
        for key in ("market_regime", "canonical_regime", "regime", "name"):
            if key in regime_value and regime_value[key]:
                return _canonical_regime_name(regime_value[key])
        return "RANGE"
    key = str(regime_value).strip().upper().replace(" ", "_")
    aliases = {
        "BULL": "TREND_UP",
        "BULLISH": "TREND_UP",
        "BEAR": "TREND_DOWN",
...
def detect_market_regime(df_1m: pd.DataFrame,
    ...
    """
    Deteksi regime pasar. Kembalikan bentuk yang kompatibel dengan kode lama
    (BULL / BEAR / SIDEWAYS / HIGH_VOL) sekaligus menambahkan canonical fields
    untuk requirement baru (TREND_UP / RANGE / BREAKOUT_* / VOLATILITY_*).
    """
    ...
    if close <= 0:
        result = {"regime": "SIDEWAYS", "market_regime": "RANGE",
                  "canonical_regime": "RANGE", "strength": "weak",
                  "conf_adjust": 12, "description": "↔️ RANGE: data tidak valid"}
        return result
    ...
    if atr_pct > 2.5:
        ...
        result = {
            ...
            "strength": strength,
            "conf_adjust": 10,
            "description": f"⚡ VOLATILITY_EXPANSION: ATR {atr_pct:.1f}% — volatilitas ekstrem, noise tinggi",
        }
        return result
    ...
    if bull_ratio >= 0.70:
        ...
        return {
            ...
            "canonical_regime": "TREND_UP",
            "strength": strength,
            "conf_adjust": -5 if strength == "strong" else 0,
            "description": (
                f"🟢 TREND_UP {strength}: {bull}/{total} sinyal bullish | "
        ...
        return {
            ...
            "canonical_regime": "TREND_DOWN",
            "strength": strength,
            "conf_adjust": +10,
            "description": (
                f"🔴 TREND_DOWN {strength}: {bear}/{total} sinyal bearish | "
                f"RSI={rsi:.0f} SMA20{'<'if sma20<sma50 else '>'}SMA50"
            ),
        }
    else:
        return {
            "regime": "SIDEWAYS",
            "market_regime": "RANGE",
            "canonical_regime": "RANGE",
            "strength": "weak",
            "conf_adjust": +12,
            "description": (
                f"↔️ RANGE: sinyal mixed ({bull}B/{bear}S/{total}T) | RSI={rsi:.0f}"
            ),
        }
...
def calc_confluence_score(df_1m: pd.DataFrame,
    ...
    Logika dibuat regime-aware: RSI rendah tidak otomatis bullish di TREND_DOWN,
    dan RSI tinggi tidak otomatis bearish di TREND_UP. Di range / breakout masuk
    ke interpretasi yang relevan sesuai kondisi pasar.
    """
    if regime is None:
        regime = detect_market_regime(df_1m, df_5m, df_15m)
    canonical_regime = _canonical_regime_name(regime)

    def _regime_rsi_vote(rsi: float, ma_trend: int, macd_h: float) -> str:
        if canonical_regime in {"TREND_UP"}:
            # Dalam tren naik, RSI rendah sering pullback, bukan bearish default.
            # Pullback yang sehat masih bullish jika MA dan MACD masih positif.
            if rsi <= 35:
                return "BULLISH" if ma_trend > 0 and macd_h > 0 else "NEUTRAL"
            if 45 <= rsi <= 65:
                return "BULLISH" if ma_trend >= 0 else "NEUTRAL"
            if rsi >= 70:
                return "BEARISH" if ma_trend < 0 or macd_h < 0 else "NEUTRAL"
            return "BULLISH"

        if canonical_regime in {"TREND_DOWN"}:
            # Dalam tren turun, RSI tinggi sering rejection / continuation, bukan bullish default.
            if rsi >= 65:
                return "BEARISH" if ma_trend < 0 and macd_h < 0 else "NEUTRAL"
            if 35 <= rsi <= 55:
                return "BEARISH" if ma_trend <= 0 else "NEUTRAL"
            if rsi <= 30:
                return "BULLISH" if ma_trend > 0 else "NEUTRAL"
            return "BEARISH"

        if canonical_regime in {"RANGE"}:
            # Range: RSI oversold = bullish mean reversion, overbought = bearish.
            if rsi <= 35:
                return "BULLISH"
            if rsi >= 65:
                return "BEARISH"
        ...
        if canonical_regime in {"BREAKOUT_UP"}:
            if rsi >= 55 and (ma_trend > 0 or macd_h > 0):
                return "BULLISH"
            if rsi <= 45 and (ma_trend < 0 or macd_h < 0):
                return "BEARISH"
            return "NEUTRAL"

        if canonical_regime in {"BREAKOUT_DOWN"}:
            if rsi <= 45 and (ma_trend < 0 or macd_h < 0):
                return "BEARISH"
            if rsi >= 55 and (ma_trend > 0 or macd_h > 0):
                return "BULLISH"
        ...
        if canonical_regime in {"VOLATILITY_EXPANSION"}:
            if rsi >= 58:
                return "BEARISH" if ma_trend <= 0 else "NEUTRAL"
            if rsi <= 42:
                return "BULLISH" if ma_trend >= 0 else "NEUTRAL"
        ...
        # fallback: interpretasi generik lama namun tetap aman
        if rsi < 45:
            return "BULLISH"
        if rsi > 55:
            return "BEARISH"
    ...
    def _tf_vote(df: Optional[pd.DataFrame], label: str) -> tuple:
        ...
        ma_trend = 1 if sma20 > sma50 else -1 if sma20 < sma50 else 0
        vote = _regime_rsi_vote(rsi, ma_trend, macd_h)

        # Momentum confirmation: bila MACD divergen dan trend kuat, vote lebih berat.
        if vote == "BULLISH" and macd_h < 0 and max(prev_h, macd_h) > 0:
            vote = "NEUTRAL"
        if vote == "BEARISH" and macd_h > 0 and min(prev_h, macd_h) < 0:
            vote = "NEUTRAL"

        if vote == "BULLISH":
            return "BULLISH", f"[{label}] BULLISH (regime={canonical_regime} RSI={rsi:.0f} MA={'up' if ma_trend > 0 else 'down' if ma_trend < 0 else 'flat'})"
        if vote == "BEARISH":
            return "BEARISH", f"[{label}] BEARISH (regime={canonical_regime} RSI={rsi:.0f} MA={'up' if ma_trend > 0 else 'down' if ma_trend < 0 else 'flat'})"
        return "NEUTRAL", f"[{label}] NEUTRAL (regime={canonical_regime} RSI={rsi:.0f})"
    ...
    if score >= 0.85:   boost = +8
    elif score >= 0.67: boost = +3
...
# ---------------------------------------------------------------------------
# ─── SETUP DETECTION ENGINE (REGIME-AWARE) ──────────────────────────────────
# ---------------------------------------------------------------------------

def detect_setup(df_1m: pd.DataFrame,
                 df_5m: Optional[pd.DataFrame] = None,
                 regime: Optional[dict] = None) -> dict:
    """
    Deteksi setup trading: TREND_CONTINUATION, PULLBACK, BREAKOUT, BREAKDOWN,
    RANGE_BOUNCE, MEAN_REVERSION, REVERSAL, LIQUIDATION_RECLAIM, SQUEEZE, NO_VALID_SETUP.

    Regime-aware: tipe setup yang valid berbeda untuk setiap regime.
    """
    if regime is None:
        regime = detect_market_regime(df_1m, df_5m)
    canonical_regime = _canonical_regime_name(regime)

    last  = df_1m.iloc[-1]
    prev  = df_1m.iloc[-2] if len(df_1m) >= 2 else last
    prev2 = df_1m.iloc[-3] if len(df_1m) >= 3 else prev
    ...
    rsi = float(last.get("rsi14", 50) or 50)
    atr = float(last.get("atr14", 0) or 0)
    bb_lower = float(last.get("bb_lower", close * 0.95) or close * 0.95)
    bb_upper = float(last.get("bb_upper", close * 1.05) or close * 1.05)

    prev_close = float(prev["close"])
    vol_ratio = (float(last.get("volume", 1) or 1) / float(prev.get("volume", 1) or 1)) if float(prev.get("volume", 1) or 1) > 0 else 1.0

    setups = []

    if canonical_regime == "TREND_UP":
        if 40 <= rsi <= 65 and close > sma20 and sma20 > sma50:
            setups.append(("TREND_CONTINUATION", 0.75, "RSI dalam zona sehat, SMA aligned up"))
        if rsi < 40 and close > sma20 and sma20 > sma50:
            setups.append(("PULLBACK", 0.65, "Pullback dengan MA intact, pullback uptrend valid"))

    elif canonical_regime == "TREND_DOWN":
        if 35 <= rsi <= 60 and close < sma20 and sma20 < sma50:
            setups.append(("TREND_CONTINUATION", 0.75, "RSI dalam zona sehat, SMA aligned down"))
        if rsi > 60 and close < sma20 and sma20 < sma50:
            setups.append(("PULLBACK", 0.65, "Pullback dengan MA intact, pullback downtrend valid"))

    elif canonical_regime == "RANGE":
        if close <= bb_lower and rsi < 30:
            setups.append(("RANGE_BOUNCE", 0.70, "Oversold di bottom Bollinger, bounce potential tinggi"))
        if close >= bb_upper and rsi > 70:
            setups.append(("RANGE_BOUNCE", 0.70, "Overbought di top Bollinger, mean reversion potential"))

    elif canonical_regime in ("BREAKOUT_UP", "BREAKOUT_DOWN"):
        close_change = (close - sma50) / sma50 * 100
        if abs(close_change) > 1.5 and vol_ratio > 1.5:
            setups.append(("BREAKOUT", 0.80, f"Breakout confirmed: {close_change:+.2f}% move, volume {vol_ratio:.2f}x"))

    if len(df_1m) >= 5:
        rev_signal, rev_reason = detect_reversal(df_1m)
        if rev_signal:
            setups.append(("REVERSAL", 0.60, rev_reason))
    ...
    if not setups:
        return {
            ...
            "score": 0.0,
            "confidence": 0.0,
            "reason": f"Tidak ada setup valid di regime {canonical_regime}",
            "required_conditions": [],
            "invalidation_conditions": [],
            "historical_stats": {},
        }

    best_setup, best_score, best_reason = max(setups, key=lambda x: x[1])
    return {
        "setup": best_setup,
        "score": round(best_score, 2),
        "confidence": round(best_score * 0.8, 2),
        "reason": best_reason,
        "required_conditions": [f"Regime: {canonical_regime}", f"RSI: {rsi:.0f}", "SMA20/50 aligned"],
        "invalidation_conditions": [f"RSI extreme (< 10 atau > 90)", "Volume turun drastis", f"Break {atr:.6f}"],
        "historical_stats": {},
    }
...
def calculate_expected_value(
    ...
    historical_win_avg: float = 1.0,
    historical_loss_avg: float = 1.0,
    fee_pct: float = 0.001,
    slippage_pct: float = 0.002,
) -> dict:
    """Hitung EV dari trade: P(win) × avg_win_R - P(loss) × avg_loss_R - fees."""
    if entry <= 0 or stop_loss >= entry or take_profit <= entry:
        return {"valid": False, "reason": "Invalid prices: SL must be < entry < TP", "ev_after_cost": 0.0, "should_trade": False}

    risk_r = (entry - stop_loss) / entry
    reward_r = (take_profit - entry) / entry
    rr_ratio = reward_r / max(risk_r, 1e-6)
    loss_probability = max(0, min(1, 1 - win_probability))
    ev_before = win_probability * reward_r * historical_win_avg - loss_probability * risk_r * historical_loss_avg
    cost_r = fee_pct * 2 + slippage_pct
    ev_after = ev_before - cost_r
    min_ev = float(os.getenv("MIN_EV_PCT", "0.003"))
    should_trade = ev_after >= min_ev

    return {
        "valid": True,
        "risk_r": round(risk_r, 6),
        "reward_r": round(reward_r, 6),
        "rr_ratio": round(rr_ratio, 2),
        "win_probability": round(win_probability, 3),
        "ev_before_cost": round(ev_before, 6),
        "ev_after_cost": round(ev_after, 6),
        "should_trade": should_trade,
        "reason": f"EV {ev_after:+.4f} {'✅ OK' if should_trade else '❌ Low'} | RR {rr_ratio:.2f}:1",
    }
...
def check_data_quality(symbol: str, df: Optional[pd.DataFrame],
                       funding: Optional[dict] = None,
                       oi_change: Optional[dict] = None) -> dict:
    """Validasi kesehatan market data sebelum AI decision. Return HOLD jika critical."""
    quality_score = 1.0
    warnings = []
    gates_passed = True

    if df is None or len(df) < 3:
        return {"score": 0.0, "quality_status": "CRITICAL", "gates_passed": False,
                "warnings": ["Data OHLCV tidak tersedia"], "reason": "Tidak ada market data"}

    last = df.iloc[-1]
    now = datetime.now(timezone.utc).timestamp() * 1000
    candle_age_ms = now - float(last.get("open_time", now) or now)

    if candle_age_ms > 120_000:
        quality_score -= 0.3
        warnings.append(f"Candle age {candle_age_ms/1000:.0f}s (stale > 2m)")

    o, h, l, c = float(last["open"]), float(last["high"]), float(last["low"]), float(last["close"])
    if not (l <= o <= h and l <= c <= h):
        quality_score = 0
        warnings.append("Invalid OHLC")
        gates_passed = False

    if float(last.get("volume", 0) or 0) <= 0:
        quality_score -= 0.2
        warnings.append("Volume 0")

    if len(df) >= 2:
        time_diff = (float(last["open_time"]) - float(df.iloc[-2]["open_time"])) / 1000
        if time_diff > 70:
            quality_score -= 0.15
            warnings.append(f"Possible missing candle ({time_diff:.0f}s gap)")

    if funding and funding.get("funding_rate_pct") is None:
        quality_score -= 0.1
        warnings.append("Funding rate unavailable")

    if oi_change and oi_change.get("oi_now") is None:
        quality_score -= 0.1
        warnings.append("OI history unavailable")

    quality_score = max(0, min(1, quality_score))
    quality_status = "CRITICAL" if quality_score < 0.5 else "WARNING" if quality_score < 0.8 else "OK"

    return {"score": round(quality_score, 2), "quality_status": quality_status, "gates_passed": gates_passed,
            "warnings": warnings, "reason": "Data quality OK" if quality_status == "OK" else f"Data {quality_status}"}


# ---------------------------------------------------------------------------
# ─── WEIGHTED CONSENSUS (MULTI-AI) ──────────────────────────────────────────
# ---------------------------------------------------------------------------

def build_weighted_consensus(
    primary_signal: dict,
    validator1: Optional[dict] = None,
    validator2: Optional[dict] = None,
    validator3: Optional[dict] = None,
    symbol: str = "",
) -> dict:
    """Build consensus dari multiple AI models dengan weight berdasarkan historis."""
    signals = [s for s in [primary_signal, validator1, validator2, validator3] if s]
    if not signals:
        return primary_signal

    symbol_weights = db_get_symbol_model_weights(symbol, lookback=30)
    default_weight = 1.0 / len(signals)

    weighted_buy = weighted_sell = weighted_hold = total_weight = 0.0
    decisions = []

    for sig in signals:
        provider = sig.get("provider", "unknown")
        weight = symbol_weights.get(provider, default_weight)
        conf = float(sig.get("confidence", 0)) / 100.0
        decision = sig.get("decision", "HOLD").upper()

        if decision == "BUY":
            weighted_buy += weight * conf
        elif decision == "SELL":
            weighted_sell += weight * conf
        else:
            weighted_hold += weight * conf
        total_weight += weight
        decisions.append({"provider": provider, "decision": decision, "confidence": sig.get("confidence", 0), "weight": round(weight, 2)})

    if total_weight == 0:
        total_weight = 1.0

    buy_score = weighted_buy / total_weight if total_weight else 0
    sell_score = weighted_sell / total_weight if total_weight else 0
    hold_score = weighted_hold / total_weight if total_weight else 0
    agreement_score = max(buy_score, sell_score, hold_score)
    disagreement_score = 1.0 - agreement_score if len(signals) > 1 else 0.0

    final_decision = "BUY" if buy_score > sell_score and buy_score > hold_score else (
        "SELL" if sell_score > buy_score and sell_score > hold_score else "HOLD")
    final_confidence = max(buy_score, sell_score, hold_score)

    return {
        "consensus_type": "weighted_voting",
        "decision": final_decision,
        "confidence": round(final_confidence * 100, 0),
        "agreement_score": round(agreement_score, 2),
        "disagreement_score": round(disagreement_score, 2),
        "provider_responses": decisions,
        "provider_count": len(signals),
        "reliability": "high" if disagreement_score < 0.3 else "medium" if disagreement_score < 0.6 else "low",
        "reason": f"{final_decision} ({final_confidence*100:.0f}%) — {len(signals)} provider agreement={agreement_score:.0%}",
    }


def db_get_symbol_model_weights(symbol: str, lookback: int = 30) -> dict:
    """Query historical accuracy untuk tiap model pada symbol ini."""
    try:
        with _db_lock:
            with sqlite3.connect(DB_FILE) as conn:
                conn.row_factory = sqlite3.Row
                ...
                """, (symbol, f"-{lookback} days")).fetchall()
        return {row["symbol"]: 1.0 for row in rows} if rows else {}
    except Exception as e:
        logger.debug(f"Symbol model weights query error: {e}")
        return {}
...
def calibrate_confidence(
    ...
    """Ubah raw AI confidence menjadi calibrated probability."""
    raw_confidence = max(0, min(1, raw_confidence))
    calibrated = (
        raw_confidence * 0.4 + setup_score * 0.2 + regime_confidence * 0.2 +
        data_quality * 0.1 + agreement_score * 0.1
    )

    return {
        "raw_confidence": round(raw_confidence, 3),
        "calibrated_probability": round(calibrated, 3),
        "calibration_status": "ESTIMATED" if raw_confidence < 0.2 or raw_confidence > 0.8 else "CALIBRATED",
    }
...
# ---------------------------------------------------------------------------
# ─── 3a. ORCHESTRATED AI DECISION PIPELINE (PHASE 2.1) ─────────────────────
# ─── Wires: data quality → regime → setup → independent AI → consensus ────
# ─── → calibration → EV → TradeDecision ──────────────────────────────────
# ---------------------------------------------------------------------------

def ask_ai_orchestrated(
    symbol: str,
    df_1m: pd.DataFrame,
    df_5m: Optional[pd.DataFrame] = None,
    df_15m: Optional[pd.DataFrame] = None,
    ...
    """
    PHASE 2.1: Complete orchestrated pipeline from market data to final TradeDecision.
    
    Pipeline:
    1. Data quality validation (BEFORE any AI calls)
    2. Regime detection & validation
    3. Detect if setup is valid for detected regime
    4. Call all AI models independently (parallel)
    5. Weighted consensus voting
    6. Confidence calibration
    7. Expected value calculation & gate
    8. Return TradeDecision object (NOT dict)
    
    Args:
        symbol: Trading pair (e.g., "BTCUSDT")
        df_1m: 1-minute OHLCV data
        df_5m: 5-minute OHLCV data (optional)
        df_15m: 15-minute OHLCV data (optional)
        funding: Futures funding data dict
        oi_change: Open interest change dict
        regime: Pre-detected regime dict
        confluence: Multi-timeframe confluence dict
        feedback: Historical feedback dict
        
    Returns:
        TradeDecision: Canonical decision object with all analysis metadata
    """
    import uuid
    ...
    try:
        # ── STEP 1: DATA QUALITY GATE (CRITICAL — reject stale/invalid data) ──
        data_quality = check_data_quality(symbol, df_1m, funding, oi_change)
        if not data_quality.get("gates_passed", False) or data_quality.get("score", 0) < 0.5:
            logger.warning(f"[{decision_id}] Data quality FAILED for {symbol}: {data_quality.get('reason', 'Unknown')}")
            return TradeDecision(
                ...
                action="HOLD",
                setup="",
                regime="DATA_QUALITY_FAILURE",
                reasoning=f"Data quality gate failed: {data_quality.get('reason', 'Unknown')}",
                dataQuality=float(data_quality.get("score", 0)),
            )
        
        logger.debug(f"[{decision_id}] Data quality OK for {symbol} (score={data_quality.get('score', 0)})")
        
        # ── STEP 2: REGIME DETECTION (if not pre-detected) ───────────────────
        if not regime or regime.get("canonical_regime") == "UNKNOWN":
            logger.warning(f"[{decision_id}] Unknown regime for {symbol}")
            return TradeDecision(
                id=decision_id,
                timestamp=now.isoformat(),
                symbol=symbol,
                ...
                setup="",
                regime="UNKNOWN",
                reasoning="Market regime could not be determined",
                dataQuality=float(data_quality.get("score", 0)),
            )
        
        canonical_regime = regime.get("canonical_regime", "UNKNOWN")
        regime_conf_adjust = regime.get("conf_adjust", 0)
        logger.debug(f"[{decision_id}] Detected regime: {canonical_regime} (conf adjust {regime_conf_adjust:+d})")
        
        # ── STEP 3: CALL ALL AI MODELS INDEPENDENTLY (parallel) ────────────────
        logger.debug(f"[{decision_id}] Calling primary AI model...")
        primary_signal = ask_ai(
            symbol, df_1m,
            df_5m=df_5m, df_15m=df_15m,
            funding=funding, oi_change=oi_change,
            regime=regime, confluence=confluence, feedback=feedback,
        )
        primary_signal["provider"] = "9Router/Primary"
        logger.debug(f"[{decision_id}] Primary signal: {primary_signal.get('decision')} (confidence {primary_signal.get('confidence')})")
        
        # Start validators in parallel (don't wait for them — use timeouts)
        validator_results = {}
        def _call_validator(name, fn):
            try:
                validator_results[name] = fn()
                validator_results[name]["provider"] = name
            except Exception as e:
                logger.error(f"[{decision_id}] Validator {name} failed: {e}")
                validator_results[name] = None
        ...
        for name, fn in validators.items():
            t = threading.Thread(target=_call_validator, args=(name, fn), daemon=True)
            t.start()
            threads.append((name, t))
        
        # Wait for validators with timeout
        for name, t in threads:
            t.join(timeout=40)
            if t.is_alive():
                logger.warning(f"[{decision_id}] Validator {name} timeout (>40s)")
        
        # ── STEP 4: WEIGHTED CONSENSUS ──────────────────────────────────────
        all_signals = {"9Router/Primary": primary_signal}
        for name, sig in validator_results.items():
            if sig is not None:
                all_signals[name] = sig
        
        consensus = build_weighted_consensus(
            primary_signal=primary_signal,
            validator1=validator_results.get("9Router/Validator-1"),
            validator2=validator_results.get("9Router/Validator-2"),
            validator3=validator_results.get("9Router/Validator-3"),
            symbol=symbol,
        )
        consensus_decision = consensus.get("decision", "HOLD").upper()
        agreement_score = consensus.get("agreement_score", 0.0)
        logger.debug(f"[{decision_id}] Consensus: {consensus_decision} (agreement={agreement_score:.2%})")
        
        # ── STEP 5: CONFIDENCE CALIBRATION ──────────────────────────────────
        last = df_1m.iloc[-1]
        price_now = float(last["close"])
        atr_now = float(last.get("atr14", 0) or 0)
        
        calibration = calibrate_confidence(
            raw_confidence=float(consensus.get("confidence", 0)) / 100.0,
            setup_score=min(0.8, agreement_score),  # Use agreement as setup quality
            regime_confidence=(regime.get("conf_adjust", 0) / 100.0) if regime else 0.5,
            data_quality=float(data_quality.get("score", 0.5)),
            agreement_score=agreement_score,
        )
        calibrated_prob = float(calibration.get("calibrated_probability", 0))
        logger.debug(f"[{decision_id}] Confidence calibrated: {float(calibration.get('raw_confidence', 0)):.2%} → {calibrated_prob:.2%}")
        
        # ── STEP 6: COMPUTE ENTRY/SL/TP FROM ATR ───────────────────────────
        sl_mult = float(os.getenv("SL_ATR_MULT", "1.0"))
        tp_mult = float(os.getenv("TP_ATR_MULT", "4.0"))
        
        entry_price = price_now
        sl_price = price_now - (sl_mult * atr_now)
        tp_price = price_now + (tp_mult * atr_now)
        
        if consensus_decision == "SELL":
            sl_price = price_now + (sl_mult * atr_now)
            tp_price = price_now - (tp_mult * atr_now)
        
        # ── STEP 7: EXPECTED VALUE GATE (REJECT IF NEGATIVE EV) ──────────────
        ev_result = calculate_expected_value(
            entry=entry_price,
            stop_loss=sl_price,
            take_profit=tp_price,
            win_probability=calibrated_prob,
            historical_win_avg=1.0,
            historical_loss_avg=1.0,
            fee_pct=float(os.getenv("FEE_PCT", "0.001")),
            slippage_pct=float(os.getenv("SLIPPAGE_PCT", "0.002")),
        )
        
        if not ev_result.get("valid", False) or not ev_result.get("should_trade", False):
            logger.warning(f"[{decision_id}] EV gate rejected: {ev_result.get('reason', 'Invalid EV')}")
            return TradeDecision(
                id=decision_id,
                timestamp=now.isoformat(),
                symbol=symbol,
                ...
                reasoning=f"EV gate: {ev_result.get('reason', 'Negative expected value')}",
                dataQuality=float(data_quality.get("score", 0)),
            )
        
        ev_after_cost = float(ev_result.get("ev_after_cost", 0))
        rr_ratio = float(ev_result.get("rr_ratio", 0))
        logger.debug(f"[{decision_id}] EV passed: {ev_after_cost:+.6f} (RR {rr_ratio:.2f}:1)")
        
        # ── STEP 8: BUILD FINAL TRADE DECISION ──────────────────────────────
        key_factors = []
        if agreement_score > 0.7:
            key_factors.append(f"Strong consensus ({agreement_score:.0%})")
        if calibrated_prob > 0.6:
            key_factors.append("High calibrated probability")
        if regime:
            key_factors.append(f"Regime-aligned setup ({canonical_regime})")
        if confluence:
            key_factors.append("Multi-timeframe confluence confirmed")
        
        invalidation_conditions = []
        if consensus_decision == "BUY":
            invalidation_conditions.append(f"Close below {sl_price:.6f} (SL)")
            invalidation_conditions.append(f"Strong reject at {tp_price:.6f}")
        elif consensus_decision == "SELL":
            invalidation_conditions.append(f"Close above {sl_price:.6f} (SL)")
            invalidation_conditions.append(f"Strong reject at {tp_price:.6f}")
        ...
        return TradeDecision(
            ...
            setup=confluence.get("detail", "") if confluence else "",
            regime=canonical_regime,
            entry=entry_price,
            stopLoss=sl_price,
            takeProfit=tp_price,
            ...
            calibratedConfidence=calibrated_prob * 100,
            reasoning=f"{consensus_decision} | Consensus {agreement_score:.0%} | EV {ev_after_cost:+.6f} | {len(all_signals)} models",
            keyFactors=key_factors,
            invalidationConditions=invalidation_conditions,
            supportingModels=[name for name, sig in all_signals.items() if sig.get("decision") == consensus_decision],
            opposingModels=[name for name, sig in all_signals.items() if sig.get("decision") != consensus_decision],
            dataQuality=float(data_quality.get("score", 0)),
        )
        
    except Exception as e:
        logger.error(f"[{decision_id}] ask_ai_orchestrated fatal error: {e}", exc_info=True)
        return TradeDecision(
            id=decision_id,
            timestamp=now.isoformat(),
            symbol=symbol,
...
def execute_binance(symbol: str, side: str, qty: float) -> dict:
    _reject_legacy_execution("Binance market order")
    client = make_binance_client()
    for attempt in range(3):
        try:
...
def place_oco_sell(symbol: str, qty: float, entry_price: float,
    ...
    _reject_legacy_execution("Binance OCO order")
...
def cancel_oco_orders(symbol: str, pos: dict) -> bool:
    ...
    _reject_legacy_execution("Binance order cancellation")
    client = make_binance_client()
...
def execute_mexc(symbol: str, side: str, qty: float) -> dict:
    ...
    _reject_legacy_execution("MEXC order")
    params = _mexc_sign({
        "symbol": symbol,
        "side": side.upper(),
        "type": "MARKET",
...
def place_mexc_tp_sl(symbol: str, qty: float, entry_price: float,
    ...
    _reject_legacy_execution("MEXC TP/SL order")
    try:
        if atr > 0:
...
def cancel_mexc_orders(symbol: str, pos: dict) -> bool:
    _reject_legacy_execution("MEXC order cancellation")
...
def execute_bybit(symbol: str, side: str, qty: float) -> dict:
    ...
    _reject_legacy_execution("Bybit order")
    body = {
        "category": "spot",
        "symbol":   symbol,
...
def place_bybit_tp_sl(symbol: str, qty: float, entry_price: float,
    ...
    """
    _reject_legacy_execution("Bybit TP/SL order")
    try:
...
def cancel_bybit_orders(symbol: str, pos: dict) -> bool:
    _reject_legacy_execution("Bybit order cancellation")
...
def execute_exchange(symbol: str, side: str, qty: float) -> dict:
    ...
    _reject_legacy_execution("exchange order")
    if ACTIVE_EXCHANGE == "mexc":
...
def place_exchange_tp_sl(symbol: str, qty: float, entry_price: float,
    ...
    _reject_legacy_execution("exchange TP/SL order")
    if ACTIVE_EXCHANGE == "mexc":
...
def cancel_exchange_orders(symbol: str, pos: dict) -> bool:
    ...
    _reject_legacy_execution("exchange order cancellation")
    if ACTIVE_EXCHANGE == "mexc":
...
def process_signal(symbol: str, trade_decision: 'TradeDecision', current_price: float, atr: float,
                   df_1m: pd.DataFrame,
                   df_5m: Optional[pd.DataFrame] = None,
                   df_15m: Optional[pd.DataFrame] = None,
                   funding: Optional[dict] = None,
    ...
    """
    PHASE 3: Auto-trading using canonical TradeDecision from ask_ai_orchestrated().
    
    TradeDecision already contains:
    - Consensus from all validators (Primary + 3 Validators in parallel)
    - Calibrated confidence & probability
    - Expected value validation
    - Full decision context & reasoning
    
    Action:
    1. Validate TradeDecision
    2. If action=HOLD → return (no trade)
    3. If action=BUY/SELL → calculate position sizing, execute, place TP/SL
    4. Record outcome for learning (PHASE 3)
    """
    decision   = trade_decision.action
    confidence = trade_decision.calibratedConfidence
    reason     = trade_decision.reasoning
    n_total = len(trade_decision.supportingModels) + len(trade_decision.opposingModels)
    model_lines = "".join(
        f"\n  • {model} → {decision}" for model in trade_decision.supportingModels
    )
    model_lines += "".join(
        f"\n  • {model} → opposite" for model in trade_decision.opposingModels
    )
    vote_summary = (
        f"{len(trade_decision.supportingModels)} mendukung / "
        f"{len(trade_decision.opposingModels)} menentang"
    )
    ...
    # Early return: HOLD decision from TradeDecision
    if decision == "HOLD":
        logger.debug(f"⏸ TradeDecision: HOLD for {symbol} — reason: {reason}")
        return
    ...
    # Consensus sudah selesai di ask_ai_orchestrated(); di sini hanya tampilkan
    # hasil final yang sudah melewati pipeline canonical.
    send_telegram_message(
        ...
        f"🤖 *Konsensus*: `{vote_summary}`",
        topic_id=_signal_topic(decision),
    )

    # ── PHASE 3: TradeDecision already has complete consensus ────────────────
    # ask_ai_orchestrated() already did:
    # 1. Data quality validation
    # 2. Regime detection
    # 3. Independent AI analysis (parallel 3 validators)
    # 4. Weighted consensus voting
    # 5. Confidence calibration
    # 6. Expected value validation
    # NO NEED to call run_multi_ai_consensus() again — that's redundant!
    
    final_decision = decision  # From TradeDecision.action
    avg_confidence = confidence  # From TradeDecision.calibratedConfidence
    ...
    # Ringkasan reasoning berasal dari TradeDecision canonical; provider detail
    # sudah tercakup dalam supporting/opposing model lists.
    reasons_summary = trade_decision.reasoning[:500]
        ...
        try:
            ...
            for symbol in pairs_snapshot:
                ...
                try:
                    ...
                    regime     = detect_market_regime(df_1m, df_5m, df_15m)
                    confluence = calc_confluence_score(df_1m, df_5m, df_15m, regime=regime)
                    ...
                    # PHASE 2.1: Call orchestrated pipeline (returns TradeDecision)
                    trade_decision = ask_ai_orchestrated(
                        symbol, df_1m,
                        df_5m=df_5m, df_15m=df_15m,
                        funding=funding, oi_change=oi_change,
                        regime=regime, confluence=confluence, feedback=feedback,
                    ...
                    # Convert TradeDecision to dict for backward compatibility
                    decision   = trade_decision.action
                    confidence = trade_decision.calibratedConfidence
                    reason     = trade_decision.reasoning
                    
                    # Keep signal dict for process_signal compatibility
                    signal = {
                        "decision": decision,
                        "confidence": confidence,
                        "reason": reason,
                    ...
                    threading.Thread(
                        ...
                        args=(symbol, trade_decision, current_price, atr, df_1m),
                        kwargs=dict(
                            df_5m=df_5m, df_15m=df_15m,
                            funding=funding, oi_change=oi_change,
                        ),
