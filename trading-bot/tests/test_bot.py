...
class TestRiskManagement(unittest.TestCase):
    ...
    def test_legacy_execution_is_fail_closed(self):
        import main as bot
        self.assertTrue(bot.LEGACY_EXECUTION_DISABLED)
        with self.assertRaisesRegex(RuntimeError, "canonical execution boundary"):
            bot.execute_exchange("BTCUSDT", "BUY", 0.01)
        with self.assertRaisesRegex(RuntimeError, "canonical execution boundary"):
            bot.place_exchange_tp_sl("BTCUSDT", 0.01, 100.0)
        with self.assertRaisesRegex(RuntimeError, "canonical execution boundary"):
            bot.cancel_exchange_orders("BTCUSDT", {})
