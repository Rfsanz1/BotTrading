...
import { getTradingDayKey } from './pnl';
...
export class PnLCalculationService {
  ...
  async calculateDailyRealizedPnL(
    userId: string,
    asOf = new Date(),
    timezone = 'UTC',
  ): Promise<number> {
    const dayKey = getTradingDayKey(asOf, timezone);
    const positions = await prisma.position.findMany({
      where: { userId, status: 'CLOSED', closedAt: { not: null } },
      select: { realizedPnL: true, closedAt: true },
    });
    return positions.reduce((sum, position) => (
      position.closedAt && getTradingDayKey(position.closedAt, timezone) === dayKey
        ? sum + (position.realizedPnL?.toNumber() ?? 0)
        : sum
    ), 0);
  }

  async calculateWeeklyRealizedPnL(
    userId: string,
    asOf = new Date(),
    timezone = 'UTC',
  ): Promise<number> {
    ...
    return positions.reduce((sum, position) => sum + (position.realizedPnL?.toNumber() ?? 0), 0);
  }
  ...
  async calculateDailyPnL(
    ...
    try {
      ...
      for (const pos of positions) {
        const date = getTradingDayKey(pos.closedAt!, 'UTC');
