...
import { calculateDirectionalPnL } from './pnl';
...
export class PositionService {
  ...
  calculateUnrealizedPnL(
    ...
    return calculateDirectionalPnL(entryPrice, currentPrice, quantity, side);
  }
    ...
    try {
      ...
      await prisma.position.update({
        ...
        data: {
          ...
          meta: {
            ...(position.meta && typeof position.meta === 'object' ? position.meta : {}),
  ...
  async closePosition(
    ...
    throw new Error(
      'Direct local position closure is disabled; submit a canonical exchange exit order and wait for an authoritative fill',
    );
  }
