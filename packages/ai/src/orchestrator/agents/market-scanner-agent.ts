...
export class MarketScannerAgent extends AgentBase {
  constructor(
    bus: any,
    memory: AIMemory,
    ...
    super('market-scanner', bus, memory);
  }

  protected async process(task: AgentTask) {
    const symbol = task.request.metadata?.symbol || 'unknown';
    const prompt = buildAgentPrompt('Market Scanner', symbol, 'market scanning', 'Scan the broader market for sectors, correlations, and potential opportunities.');
    const request: AIRequest = createAgentRequest(task, 'market-scanner', prompt);
    const response = await this.providerManager.execute(request, this.providerManager.getBestProvider(request));

    const summary = `Market scanner found ${response.content.slice(0, 120)}`;
    await this.persistMemory({
      type: 'NOTE',
      title: `Market scan ${symbol}`,
      content: response.content,
      metadata: { symbol, category: 'market-scanner' },
    });
