...
export class AITradingBrain {
  ...
  constructor(config: Partial<BrainConfig> = {}) {
    ...
    this.learningSystem = new LearningSystem(undefined, prisma as any);
  ...
  private async generateRecommendation(
    ...
    const modelPerformance = this.learningSystem.getModelPerformance();
    const learningWeight = modelPerformance.length > 0
      ? modelPerformance.reduce((sum, metric) => sum + this.learningSystem.getModelWeight(metric.provider, metric.model, input.symbol, metric.regime, metric.setup), 0) / modelPerformance.length
      : 0.5;
    const baseConfidence = Math.min(1, (aggregated.confidence + decision.confidence + learningWeight) / 3);
  ...
  recordTradeResult(tradeHistory: any): void {
    if (!tradeHistory || !tradeHistory.decisionId) {
      return;
    }

    const normalized = {
      decisionId: tradeHistory.decisionId,
      timestamp: tradeHistory.timestamp ?? Date.now(),
      symbol: tradeHistory.symbol ?? 'UNKNOWN',
      provider: tradeHistory.provider ?? 'openai',
      model: tradeHistory.model ?? 'default',
      regime: tradeHistory.regime,
      setup: tradeHistory.setup,
      outcome: tradeHistory.outcome ?? 'neutral',
      entryPrice: tradeHistory.entryPrice ?? 0,
      exitPrice: tradeHistory.exitPrice ?? 0,
      realizedPnL: tradeHistory.realizedPnL ?? 0,
      realizedR: tradeHistory.realizedR ?? 0,
      fees: tradeHistory.fees ?? 0,
      slippage: tradeHistory.slippage ?? 0,
      holdingTimeMs: tradeHistory.holdingTimeMs ?? 0,
      exitReason: tradeHistory.exitReason,
      decision: tradeHistory.decision ?? 'HOLD',
    };

    this.learningSystem.recordTradeResult(normalized);
  }
  ...
  async getLearningInsights(symbol: string): Promise<any> {
    return this.learningSystem.getLearningInsights(symbol);
  }
