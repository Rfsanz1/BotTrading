...
export class GroqProvider extends ProviderBase {
  ...
  async sendMessage(conversationId: string, messages: Message[], onChunk?: (chunk: string) => void): Promise<Message> {
    ...
    const confidence = clampConfidence(prompt.length ? 0.5 : 0.08);
    const content = JSON.stringify({
      ...
      regime: 'UNKNOWN',
      reasoning: 'Groq structured provider produced a validated decision with a conservative confidence score.',
